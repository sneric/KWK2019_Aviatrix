...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        // add current: myPlane.location to distance variable (iteration 1)
        let distance = myPlane.distanceTo(current: myPlane.location, target: city)
...
func refuel(myPlane : Aviatrix) {
    ...
    // iteration 2 - add to get print statements below to work
...
var plane = Aviatrix(userName: "Eric")
